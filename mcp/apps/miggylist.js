...
export function buildServer(userId, apiUrl = "http://localhost:3001", cfHeaders = {}) {
  ...
  server.resource("boards", "miggylist://boards", async () => {
    ...
    return { contents: [{ uri: "miggylist://boards", mimeType: "application/json", text: JSON.stringify(boards, null, 2) }] };
  });

  server.resource("inbox", "miggylist://inbox", async () => {
    const data = await api("GET", "/miggylist-api/inbox");
    return { contents: [{ uri: "miggylist://inbox", mimeType: "application/json", text: JSON.stringify(data, null, 2) }] };
  });

  server.resource("stats", "miggylist://stats", async () => {
    const data = await api("GET", "/miggylist-api/stats");
    return { contents: [{ uri: "miggylist://stats", mimeType: "application/json", text: JSON.stringify(data, null, 2) }] };
  });
  ...
  server.tool("get_inbox", "Get all Inbox-status tasks across all boards, with board and group context.", {}, async () => {
    const data = await api("GET", "/miggylist-api/inbox");
    return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
  });
  ...
  server.tool(
    ...
    "Create a new task in a specific group.",
    {
      ...
      status: z.enum(["Inbox", "Spark", "Slog", "In Progress", "Done"]).optional().describe(`Task status. One of: ${STATUS_VALUES.join(", ")}. Defaults to Inbox.`),
      ...
      prompt: z.string().optional().describe("AI prompt for a downstream agent to execute this task"),
  ...
  server.tool(
    ...
    "Update fields on an existing task.",
    {
      ...
      status: z.enum(["Inbox", "Spark", "Slog", "In Progress", "Done"]).optional().describe(`New status. One of: ${STATUS_VALUES.join(", ")}`),
      priority: z.enum(["Low", "Medium", "High", "Critical"]).optional().describe(`New priority. One of: ${PRIORITY_VALUES.join(", ")}`),
  ...
  server.tool(
    ...
    async ({ item_id, to_group_id, to_index }) => {
      const item = await api("PUT", `/miggylist-api/items/${item_id}/move`, { toGroupId: to_group_id, toIndex: to_index });
      return { content: [{ type: "text", text: JSON.stringify(item, null, 2) }] };
    }
  );
